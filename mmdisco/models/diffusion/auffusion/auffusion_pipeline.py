...
class ConditionAdapter(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.proj = nn.Linear(
            self.config["condition_dim"], self.config["cross_attention_dim"]
        )
    ...
    def from_pretrained(cls, pretrained_model_name_or_path):
        ...
        instance = cls(config)
        instance.load_state_dict(torch.load(ckpt_path, map_location="cpu"))
    ...
    def save_pretrained(self, pretrained_model_name_or_path):
        ...
        print(
            f"SAVED: ConditionAdapter {self.config['model_name']} to {pretrained_model_name_or_path}"
        )
...
class AuffusionPipeline(DiffusionPipeline):
    ...
    def from_pretrained(
        ...
        vocoder = Generator.from_pretrained(
            pretrained_model_name_or_path, subfolder="vocoder"
        ).to(device, dtype)

        text_encoder_list, tokenizer_list, adapter_list = [], [], []

        condition_json_path = os.path.join(
            pretrained_model_name_or_path, "condition_config.json"
        )
        ...
        for i, condition_item in enumerate(condition_json_list):
            ...
            text_encoder_path = os.path.join(
                pretrained_model_name_or_path, condition_item["text_encoder_name"]
            )
            ...
            text_encoder = text_encoder_cls.from_pretrained(text_encoder_path).to(
                device, dtype
            )
            ...
            adapter_path = os.path.join(
                pretrained_model_name_or_path, condition_item["condition_adapter_name"]
            )
    ...
    def _encode_prompt(
        ...
        def get_prompt_embeds(prompt_list, device):
            ...
            for prompt in prompt_list:
                ...
                for j in range(len(self.text_encoder_list)):
                    ...
                    input_ids = self.tokenizer_list[j](
                        prompt, return_tensors="pt"
                    ).input_ids.to(device)
                    cond_embs = self.text_encoder_list[j](
                        input_ids[:, : self.tokenizer_model_max_length]
                    ).last_hidden_state  # [bz, text_len, text_dim]
                    ...
                    if cond_embs.shape[1] < self.tokenizer_model_max_length:
                        cond_embs = torch.functional.F.pad(
                            ...
                            (
                                ...
                                0,
                                0,
                                self.tokenizer_model_max_length - cond_embs.shape[1],
                            ),
                            value=0,
                        )
                    else:
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            if negative_prompt is None:
                negative_prompt_embeds = torch.zeros_like(prompt_embeds).to(
