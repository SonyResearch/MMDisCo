...
Multi-modal gaussian have been added, as well as zero-shot conditional generation.
"""
...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        video_output, audio_output = model(
            x["video"], x["audio"], self._scale_timesteps(t), **model_kwargs
        )  # when ddim, t is not mapped
        ...
        video_mean, video_variance, video_log_variance, pred_video_xstart = (
            get_variance(video_output, x["video"])
        )
        audio_mean, audio_variance, audio_log_variance, pred_audio_xstart = (
            get_variance(audio_output, x["audio"])
        )
    ...
    def p_sample(
        ...
        video_sample = (
            out["mean"]["video"]
            + video_nonzero_mask
            * th.exp(0.5 * out["log_variance"]["video"])
            * video_noise
        )
        audio_sample = (
            out["mean"]["audio"]
            + audio_nonzero_mask
            * th.exp(0.5 * out["log_variance"]["audio"])
            * audio_noise
        )

        return {
            "sample": {"video": video_sample, "audio": audio_sample},
            "pred_start": {
                "video": out["pred_xstart"]["video"],
                "audio": out["pred_xstart"]["audio"],
            },
            "pred_noise": {
                "video": out["model_predict"]["video"],
                "audio": out["model_predict"]["audio"],
            },
        }
    ...
    def p_sample_loop_progressive(
        ...
        if device is None:
            raise ValueError("device must be passed")
    ...
    def conditional_p_sample_loop(
        ...
        final = None
        if class_scale == 0:
            conditional_p_sample_loop_progressive_func = (
                self.conditional_p_sample_loop_progressive_unscale
            )
        else:
            conditional_p_sample_loop_progressive_func = (
                self.conditional_p_sample_loop_progressive_scale
            )
    ...
    def conditional_p_sample_loop_progressive_scale(
        ...
        for i in indices:
            ...
            if video_condition is not None:
                ...
                previous_step_condition = self.q_sample(
                    video_condition, t - 1, noise=noise[condition]
                )
            if audio_condition is not None:
                condition = "audio"
                target = "video"
                ...
                previous_step_condition = self.q_sample(
                    audio_condition, t - 1, noise=noise[condition]
                )

            with th.enable_grad():
                none_zero_mask = (
                    (t != 0).float().view(-1, *([1] * (len(x[target].shape) - 1)))
                )
                ...
                loss = mean_flat(
                    (previous_step_pred[condition] - previous_step_condition) ** 2
                )
                loss_scale = 1.0
                if use_fp16 == True:
                    loss_scale = 2**20
                ...
                x[target] = (
                    previous_step_pred[target]
                    - none_zero_mask * grad * class_scale * self.sqrt_alphas_cumprod[i]
                )
    ...
    def ddim_sample(
        ...
        eps = {}
        eps["video"] = self._predict_eps_from_xstart(
            x["video"], t, out["pred_xstart"]["video"]
        )
        eps["audio"] = self._predict_eps_from_xstart(
            x["audio"], t, out["pred_xstart"]["audio"]
        )

        alpha_bar = {}
        alpha_bar["video"] = _extract_into_tensor(
            self.alphas_cumprod, t, x["video"].shape
        )
        alpha_bar["audio"] = _extract_into_tensor(
            self.alphas_cumprod, t, x["audio"].shape
        )

        alpha_bar_prev = {}
        alpha_bar_prev["video"] = _extract_into_tensor(
            self.alphas_cumprod_prev, t, x["video"].shape
        )
        alpha_bar_prev["audio"] = _extract_into_tensor(
            self.alphas_cumprod_prev, t, x["audio"].shape
        )

        sigma = {}
        sigma["video"] = (
            eta
            * th.sqrt((1 - alpha_bar_prev["video"]) / (1 - alpha_bar["video"]))
            * th.sqrt(1 - alpha_bar["video"] / alpha_bar_prev["video"])
        )
        sigma["audio"] = (
            eta
            * th.sqrt((1 - alpha_bar_prev["audio"]) / (1 - alpha_bar["audio"]))
        ...
        sample = {}
        sample["video"] = (
            mean_pred["video"] + nonzero_mask["video"] * sigma["video"] * noise["video"]
        )
        sample["audio"] = (
            mean_pred["audio"] + nonzero_mask["audio"] * sigma["audio"] * noise["audio"]
        )
    ...
    def ddim_reverse_sample(
        ...
        eps = {}
        eps["video"] = (
            _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x["video"].shape)
            * x["video"]
            - out["video"]["pred_xstart"]
        ) / _extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x["video"].shape)

        eps["audio"] = (
            _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x["audio"].shape)
            * x["audio"]
            - out["audio"]["pred_xstart"]
        ) / _extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, x["audio"].shape)

        alpha_bar_next = {}
        alpha_bar_next["video"] = _extract_into_tensor(
            self.alphas_cumprod_next, t, x["video"].shape
        )
        alpha_bar_next["audio"] = _extract_into_tensor(
            self.alphas_cumprod_next, t, x["audio"].shape
        )
    ...
    def _vb_terms_bpd(
        ...
        """
        video_true_mean, _, video_true_log_variance_clipped = (
            self.q_posterior_mean_variance(
                x_start=x_start["video"], x_t=x_t["video"], t=t
            )
        )
        audio_true_mean, _, audio_true_log_variance_clipped = (
            self.q_posterior_mean_variance(
        ...
        true_log_variance_clipped = {
            "video": video_true_log_variance_clipped,
            "audio": audio_true_log_variance_clipped,
        }
        ...
        for key in ["video", "audio"]:
            kl[key] = normal_kl(
                true_mean[key],
                true_log_variance_clipped[key],
                out["mean"][key],
                out["log_variance"][key],
            )
            ...
            decoder_nll[key] = -discretized_gaussian_log_likelihood(
                x_start[key],
                means=out["mean"][key],
                log_scales=0.5 * out["log_variance"][key],
            )
    ...
    def multimodal_training_losses(
        ...
        if noise is None:
            noise = {
                "video": th.randn_like(video_start),
        ...
        video_output, audio_output = model(
            video_t, audio_t, self._scale_timesteps(t), **model_kwargs
        )
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            if self.model_var_type in [
                ...
                video_output, video_var_values = th.split(
                    video_output, video_start.shape[2], dim=2
                )
                audio_output, audio_var_values = th.split(
                    audio_output, audio_start.shape[1], dim=1
                )
                # Learn the variance using the variational bound, but don't let
                # it affect our mean prediction.
                video_frozen_out = th.cat(
                    [video_output.detach(), video_var_values], dim=2
                )
                audio_frozen_out = th.cat(
                    [audio_output.detach(), audio_var_values], dim=1
                )
