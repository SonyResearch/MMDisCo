...
class ResBlock(TimestepBlock):
    ...
    def _forward(self, video, audio, emb):
        ...
        if self.use_scale_shift_norm:
            # use the same embed
            video_out_norm, video_out_rest = (
                self.video_out_layers[0],
                self.video_out_layers[1:],
            )
            ...
            audio_out_norm, audio_out_rest = (
                self.audio_out_layers[0],
                self.audio_out_layers[1:],
            )
...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.audio_proj_out = zero_module(
            AudioConv(self.channels, self.channels, 1, conv_type="linear")
        )
        self.va_index = None
        self.av_index = None
    ...
    def attention_index(self, audio_size, video_size, device):
        ...
        if self.av_index == None:
            ...
            av_index_y = (
                th.arange(0, f).unsqueeze(-1).repeat(1, audio_len_perf).view(-1, 1)
            )
        ...
        if attention_index_a.shape[0] < l:
            attention_index_a = th.cat(
                [
                    attention_index_a,
                    attention_index_a[-1 * (l - attention_index_a.shape[0]) :],
                ],
                dim=0,
            )
    ...
    def _forward(self, video, audio):
        ...
        video_h, audio_h = self.attention(
            qkv, attention_index_v, attention_index_a, h * w, int(l / f)
        )
...
class InitialBlock(nn.Module):
    def __init__(
        ...
        self.audio_conv = AudioConv(
            audio_in_channels, audio_out_channels, kernel_size, conv_type="linear"
        )
...
class MultimodalUNet(nn.Module):
    ...
    :cross_attention_resolutions: a collection of downsample rates at which cross
        ...
        will be used.
    :cross_attention_windows: a collection of cross-attention window sizes, corresponding
        ...
        for key, val in self.state_dict().items():
            if key in state_dict.keys():
                if val.shape == state_dict[key].shape:
                    ...
                    state_dict.pop(key)
                    logger.log(
                        "{} not matchable with state_dict with shape {}".format(
                            key, val.shape
                        )
                    )
            else:
