    ...
    def __init__(
        ...
        **kwargs,
        ...
        if cross_attend and not only_cross:
            ...
            default_block = ("a", "f")  # TODO:  -> ("a", "a", "f") as frieren?
...
class TransformerWrapperNoTokenEmb(nn.Module):
    def __init__(
        self,
        *,
        model_dims,
        ...
        attn_layers = Encoder(dim=model_dims, depth=n_attn_layers)
        ...
        scale = emb_dim**-0.5
        if use_class_token:
            self.cls_emb = nn.Parameter(scale * torch.randn((1, 1, emb_dim)))
        ...
        )  # TODO: -> conv 1d? according to Frieren
        self.attn_layers = attn_layers
        self.norm = nn.LayerNorm(dim)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.use_class_token:
            cls_emb = torch.tile(
                self.cls_emb.to(dtype=x.dtype, device=x.device), (x.shape[0], 1, 1)
            )
            x = torch.concat([cls_emb, x], dim=1)
        ...
        x, intermediates = self.attn_layers(
            x, mask=mask, mems=None, return_hiddens=True, **kwargs
        )
